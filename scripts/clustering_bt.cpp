...
void modCb(const visualization_msgs::InteractiveMarkerFeedbackConstPtr &feedback)
{
    ...
    for (size_t i = 0; i < server->size(); i++)
    {
        ...
        im.controls[1].markers[2].color.a = 1;
        server->insert(im);
    }
    server->applyChanges();
...
void reqCb(const visualization_msgs::InteractiveMarkerFeedbackConstPtr &feedback)
{
    ...
    hrov_martech2023::BaseGoal bgoal;
    bgoal.position.x = goal.translation().x();
    bgoal.position.y = goal.translation().y();
    bgoal.position.z = goal.translation().z();
    bgoal.yaw = atan2(goal.rotation()(1, 0), goal.rotation()(0, 0));
    std::cout << "requesting to:\n"
              << bgoal << "\n";
    goalPub.publish(bgoal);
