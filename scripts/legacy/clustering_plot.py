...
def plot_covar(fig: go.Figure, centroides, covar_matrix):
    ...
    fig.add_trace(
        go.Scatter3d(
            x=[centroides[0], (evec[0][min]) * 0.5 + centroides[0]],
...
fig.add_trace(
    go.Scatter3d(
        ...
        z=X[:, 2],
        mode="markers",
        marker=dict(size=8, color=dpgmm.predict(X), colorscale="Viridis", opacity=0.8),
        name="Puntos",
    )
)
...
fig.update_layout(
    ...
    title=dict(
        text="Frontiers: All directions3 <br><sup>Clustering Params: default</sup>",
        font=dict(size=50),
        automargin=True,
        yref="paper",
    )
