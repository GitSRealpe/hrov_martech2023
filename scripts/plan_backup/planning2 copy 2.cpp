...
public:
    ...
    virtual bool isValid(const ob::State *state) const
    {
        ...
        auv_co_->setRotation(Eigen::AngleAxisf(stt.reals().at(3), Eigen::Vector3f::UnitY()).matrix());
...
int main(int argc, char **argv)
{
    ...
    // fcl::Transform3f auv_tf(Eigen::Isometry3f().Identity());
    // auv_tf.matrix().block<3, 3>(0, 0) = Eigen::AngleAxisf(1.57, Eigen::Vector3f::UnitY()).matrix();
    // auv_tf.matrix().block<3, 1>(0, 3) = Eigen::Vector3f(1, 2, 3);
    // Eigen::IOFormat fmt(3, 0, ", ", ";\n", "[", "]", "[", "]");
    // std::cout << auv_tf.matrix().format(fmt) << "\n";

    // std::shared_ptr<fcl::Boxf> auv_box(new fcl::Boxf(2, 2, 2));
    // std::shared_ptr<fcl::CollisionObjectf> auv_co(new fcl::CollisionObjectf(auv_box, auv_tf));
    ...
    // std::cout << "rango configurado " << planner->getRange() << "\n";
    ...
    if (solved)
    {
        ...
        pose = Eigen::AngleAxisd(M_PI / 4, Eigen::Vector3d::UnitY());
        pose.translation() = Eigen::Vector3d(0.1, 0.1, 0.1);
        ...
        // std::cout << pdef->getSolutionPath()->as<og::PathGeometric>()->getStateCount() << "\n";
        // og::PathGeometricPtr pathr = std::make_shared<og::PathGeometric>(pdef->getSolutionPath()->as<og::PathGeometric>());
        ...
        {
            ob::ScopedState<ob::CompoundStateSpace> stt(navSpace, pd.getVertex(i).getState());
            // stt.print();
            punto.translation().x() = stt.reals().at(0);
            punto.translation().y() = stt.reals().at(1);
            punto.translation().z() = stt.reals().at(2);
